...
//@ts-ignore
import {PloverTranslations} from "./../data/plover-translations.mjs";
import {N, initializeButtons} from "./utils/util.mjs";
/**
 * @typedef {{
...
export class TypeJig {
    ...
    static shortestTranslations = function(t) {
        // Get the custom dictionarys from localstorage
        const customDictionariesString =
            localStorage.getItem("customDictionaries");
        ...
        if (customDictionariesString) {
            JSON.parse(customDictionariesString).forEach((dictionary) => {
                customDictionaries.push(
                    TypeJig.processTranslations(dictionary),
                );
            });
        }
    ...
    gradeTypeVsResult(typedWords, expectedWords = this.exercise.words) {
        ...
        for (const i of typedWords) {
            ...
            for (
                ...
                const offsetExpectedWord =
                    expectedWords[expectedIndex + offset];
        ...
        for (let i = 0; i < typedWords.length; i++) {
            ...
            if (word.addedWords) {
                for (const element of word.addedWords) {
                    ...
                    addedWordNode.appendChild(
                        document.createTextNode(addedWord),
                    );
    ...
    addWordToExercise(
        ...
        if (
            this.typedWords[i] &&
            this.typedWords[i].typed.length > word.length
        ) {
            N(
                finalObject,
                {
    ...
    showResults(persistantData) {
        ...
        let sortedPersistantWordData = [...persistantData].sort(function(
            a,
            b,
        ) {
        ...
        sortedPersistantWordData.forEach((element) => {
            ...
            newCell.innerHTML =
                ((element.duration ?? 0) * 1000).toFixed(0) + "ms";
            newRow.appendChild(newCell);
            newCell = document.createElement("td");
    ...
    renderChart() {
        ...
        combinedData.sort(
            (a, b) => a.lastKnownTimeStamp - b.lastKnownTimeStamp,
        );
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            title: function(context) {
                                ...
                                return [
                                    // @ts-ignore
                                    all.datasets[tooltipItem[0].datasetIndex]
                                        // @ts-ignore
                                        .data[tooltipItem[0].index].expected,
                                ];
                            },
