...
// @ts-ignore
import {PloverPunctuation} from "../../data/plover-translations.mjs";

import {analyze} from "../spectra/spectra.mjs";
/**
 * @typedef {import("../spectra/spectra.js").AnalyzeResult} AnalyzeResult
 */
...
/**
 *
 * @param {import("../spectra/spectra.js").ProcessedResult} strokeAnalysis
 * @param {*} showEmpty
 */
StrokeGroup.prototype.set = function(strokeAnalysis, showEmpty) {
    this.show();
    console.log("Setting StrokeGroup", strokeAnalysis, showEmpty);
    ...
    // if (showEmpty) {
    const typedStrokes = strokeAnalysis.outline.split("/");
...
StrokeGroup.prototype.hide = function() {
    this.container.style.display = "none";
};

StrokeGroup.prototype.show = function() {
    this.container.style.display = "inline-block";
};

export class StenoDisplay {
    constructor(container, translations, showEmpty) {
        console.log(
            "Creating StenoDisplay",
            container,
            translations,
            showEmpty,
        );
        ...
        //@ts-ignore
        this.hintComputer = new Worker(new URL("../precomputeHints.mjs", import.meta.url));
        ...
        // const urls = [
        //     "scripts/type-jig.mjs",
        //     "data/plover-translations.mjs",
        //     "scripts/spectra/rules.mjs",
        //     "scripts/spectra/spectra.mjs",
    ...
    startupPrecompute(fullText, options) {
        ...
        const data = {
            ...
            options: options,
            punctuation: PloverPunctuation,
        };
        this.hintComputer.postMessage(data);
    }
    async update(text, x, y) {
        text = text || "";
        ...
        if (text !== this.lastText) {
            ...
            for (let i = words.length; i > 0; i--) {
                ...
                const analyzeResult = await this.lookup(subString);
            ...
            return [
                {
                    title: "Number",
                    res: {
                        outline: this.numberStrokes(text),
                        rules: [],
                    },
                },
            ];
        }
        //If the text is Psuedo Steno, then we can just return the text
        if (/^([STKPWHRAO*EUFRPBLGTSDZ]+-?)+$/.test(text)) {
            console.log("Pseudo Steno");
            return [{
                title: "Pseudo Steno",
                res: {
                    outline: text,
        ...
        for (const dictionary of this.pseudoStenoFor) {
            ...
            if (!strokes) {
                ...
                for (let key in PloverPunctuation) {
                    ...
                    const value = PloverPunctuation[key];
                ...
                if (addStrokeBefore) {
                    strokes = strokes.map(
                        (text) => addStrokeBefore + "/" + text,
                    );
                }
                if (addStrokeAfter) {
                    strokes = strokes.map(
                        (text) => text + "/" + addStrokeAfter,
                    );
                }
            ...
            while (true) {
                try {
                    analysisResult = analyze(strokes, text);
                    break;
                } catch (e) {
                    console.log("Error analyzing", strokes, text, e);
     ...
     * @param {*} showEmpty
     */
    set(strokeAnalysis, showEmpty) {
        console.trace("Setting Display", strokeAnalysis, showEmpty);
    ...
    showAlternatives(stenoStroke, firstSep, i0) {
        if (!i0) {
            i0 = 0;
        }
    ...
    showTranslation(pseudoSteno, i0, separator, rules) {
        const strokes = pseudoSteno.split("/");
...
class Stroke {
    constructor(container) {
        ...
        const vowel = document.createElement("tr");
        this.keys.appendChild(upper);
        this.keys.appendChild(lower);
        this.keys.appendChild(vowel);
        const upperKeys = ["S", "T", "P", "H", "*", "F", "P", "L", "T", "D"];
        const lowerKeys = ["K", "W", "R", "R", "B", "G", "S", "Z"];
        const vowelKeys = ["", "A", "O", "", "E", "U"];
        const upperCells = addCells(upper, upperKeys);
        const lowerCells = addCells(lower, lowerKeys);
        const vowelCells = addCells(vowel, vowelKeys);
    ...
    applyRules(rules) {
        ...
        appliedRules.forEach((rule) => {
            ...
            const punctuationLeftMatches = ["TP", "KW", "KR"];

            const punctuationRightMatches = ["PL", "BG", "LT", "GS"];
...
function removeClass(elt, className) {
    const regex = new RegExp("(^|\\s)" + className + "\\b", "g");
    const other = elt.className.replace(regex, "");
    elt.className = other;
}
...
function pseudoStrokeToSteno(stroke) {
    const match = separationRE.exec(stroke);
    const b = match[1];
    const v = match[2];
    const e = match[3];
    const left = b.replace(leftRE, function(m) {
        return leftFromPseudo[m] || m;
    });
    const vowel = v.replace(vowelRE, function(m) {
        return vowelFromPseudo[m] || m;
    });
    const right = e.replace(rightRE, function(m) {
...
// const preOrPostChars = new RegExp(
//     "^([^" +
//         unicodeWordChar +
//         "]*)" +
//         "([" +
